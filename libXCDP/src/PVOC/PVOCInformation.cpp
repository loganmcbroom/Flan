...
PVOC::Salience PVOC::getSalience( Channel channel, Frequency minFreq, Frequency maxFreq, XCDP_CANCEL_ARG_CPP ) const
	{
	if( isNull() )
		return Salience();
...
PVOC::Salience PVOC::getSalience_gpu( Channel channel, Frequency minFreq, Frequency maxFreq, XCDP_CANCEL_ARG_CPP ) const
	{
	if( isNull() )
		return Salience();
...
std::vector<PVOC::Contour> PVOC::getContours( Channel channel, Frequency minFreq, Frequency maxFreq, 
	Frame filterShort, float filterQuiet, XCDP_CANCEL_ARG_CPP ) const
	{
	...
	for( Frame frame = 0; frame < salience.numFrames; ++frame )
		{
		XCDP_CANCEL_POINT( std::vector<PVOC::Contour>() );
...
PVOC PVOC::prism( PrismFunc f, bool perNote, XCDP_CANCEL_ARG_CPP ) const
	{
