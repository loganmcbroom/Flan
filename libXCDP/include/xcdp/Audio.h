...
public:
	 ...
	 */
	float getTotalEnergy( XCDP_CANCEL_ARG ) const;
	 ...
	 *  \param height The bmp height.
	 */
	Audio saveToBMP( const std::string & filename, Interval I = Interval( 0, -1 ), Pixel width = -1, Pixel height = -1, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	PVOC convertToPVOC( Frame windowSize = 2048, Frame hop = 128, Frame fftSize = 4096, std::shared_ptr<FFTHelper> fft = nullptr, XCDP_CANCEL_ARG ) const;

	/** See Audio::convertToPVOC. This explicitly calls a cpu-based implementation.
	 */
	PVOC convertToPVOC_cpu( Frame windowSize = 2048, Frame hop = 128, Frame fftSize = 4096, std::shared_ptr<FFTHelper> fft = nullptr, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Func1x1 convertToFunction( Time granularity = .001f, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio convertToMidSide( XCDP_CANCEL_ARG ) const;

	/** This converts back to Left/Right from Mid/Size. See Audio::convertToMidSide. 
	 */
	Audio convertToLeftRight( XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio convertToStereo( XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio convertToMono( XCDP_CANCEL_ARG ) const;
	 ...
	 */
	float getLocalWavelength( Channel channel, Frame start, Frame windowSize = 2048, std::shared_ptr<FFTHelper> fft = nullptr, XCDP_CANCEL_ARG ) const;
	 ...
	 *	\param fft The fft object. See xcdp::FFTHelper.
	 */
	std::vector<float> getLocalWavelengths( Channel channel, Frame start = 0, Frame end = -1, Frame windowSize = 2048, Frame hop = 128, std::shared_ptr<FFTHelper> fft = nullptr, XCDP_CANCEL_ARG ) const;
	...
	float getAverageWavelength( Channel, float minActiveRatio = 0, float maxLengthSigma = -1, Frame start = 0, Frame end = -1, 
		Frame windowSize = 2048, Frame hop = 128, std::shared_ptr<FFTHelper> fft = nullptr, XCDP_CANCEL_ARG ) const; // Averages getLocalWavelengths
	 ...
	 *	\param fft External fft handler. IMPORTANT: if using this, the fft size needs to be found with autocorrelationFFTSize( windowSize ) in DSPUtility.
	 */
	Frequency getLocalFrequency( Channel channel, Frame start = 0, Frame windowSize = 2048, std::shared_ptr<FFTHelper> fft = nullptr, XCDP_CANCEL_ARG ) const;
	...
	std::vector<Frequency> getLocalFrequencies( Channel channel, Frame start = 0, Frame end = -1, Frame windowSize = 2048, 
		Frame hop = 128, std::shared_ptr<FFTHelper> fft = nullptr, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio setVolume( Func1x1 level, XCDP_CANCEL_ARG ) const;

	/** Time shifting.
	 *	\param shift How much to shift the input.
	 */
	Audio shift( Time shift, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio waveshape( Func1x1 shaper, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio pan( Func1x1 lrAmount, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio widen( Func1x1 widenAmount, XCDP_CANCEL_ARG ) const;

	/** This reverses the Audio.
	 */
	Audio reverse( XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio cut( Time start, Time end, Time startFade = 0, Time endFade = 0, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio cutFrames( Frame start, Frame end, Frame startFade = 0, Frame endFade = 0, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio repitch( Func1x1 factor, Time granularity = .001f, uint32_t qual = 0, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio convolve( const std::vector<Func1x1> & ir, XCDP_CANCEL_ARG ) const;

	/** This is normal convolution between Audio.
	 *	\param ir An impulse response.
	 */
	Audio convolve( const Audio & ir, XCDP_CANCEL_ARG ) const;
	...
	Audio fade( Time start = 16.0f/48000.0f, Time end = 16.0f/48000.0f, 
		Interpolator interp = Interpolators::sqrt, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio lowPass( Func1x1 cutoff, uint32_t taps = 64, XCDP_CANCEL_ARG ) const;
	 ...
	 */
	Audio iterate( uint32_t n, Mod mod = nullptr, bool feedback = false, XCDP_CANCEL_ARG ) const;
	...
	Audio delay( Time length, Func1x1 delayTime, Func1x1 decay = .5, 
		Audio::Mod mod = nullptr, XCDP_CANCEL_ARG ) const;
	...
	Audio texture( Time length, Func1x1 eventsPerSecond, Func1x1 scatter = 0, Mod mod = nullptr, 
		bool feedback = false, XCDP_CANCEL_ARG ) const;
	...
	Audio grainSelect( Time length, Func1x1 grainsPerSecond, Func1x1 scatter, Func1x1 selection, 
		Func1x1 grainLength = 0.1, Func1x1 fade = 0.05, Mod mod = nullptr, XCDP_CANCEL_ARG ) const;
	 ...
	 *  \param fade The start and end fade time of each slice.
	 */
	Audio rearrange( Time sliceLength, Time fade = 0.05, XCDP_CANCEL_ARG ) const;
	 ...
	 *	\param ins The Audio to mix.
	 */
	static Audio join( const std::vector<Audio> & ins, Time fade = 0, XCDP_CANCEL_ARG );
	...
	static Audio select( const std::vector<Audio> & ins, Func1x1 selection,
		std::vector<Time> startTimes = std::vector<Time>(), XCDP_CANCEL_ARG );
};
