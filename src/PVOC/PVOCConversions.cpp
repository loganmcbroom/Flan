...
Audio PVOC::convertToAudio_cpu( std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP ) const
	{
	flan_FUNCTION_LOG;
	...
	const float windowScale = 2.67f / ( dftSize * windowSize / hopSize ); // I don't know why, converting audio->flan->audio reduces volume by an input specific amount, usually about 2.67
...
Audio PVOC::convertToAudio( std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP ) const
	{
...
Graph PVOC::convertToGraph( Rect D, Pixel width, Pixel height, float timelineScale, flan_CANCEL_ARG_CPP ) const
	{
...
const PVOC & PVOC::saveToBMP( const std::string & fileName, Rect D, Pixel width, Pixel height, flan_CANCEL_ARG_CPP ) const
	{
	flan_FUNCTION_LOG;
