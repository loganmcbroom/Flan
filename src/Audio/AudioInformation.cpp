...
static std::vector<float> compute_d( const float * in, Frame n, std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP )
	{
    const size_t fftSize = n; //std::pow( 2, 1 + (int) std::ceil( std::log2( n ) ) );
	if( !fft ) fft = std::make_shared<FFTHelper>( fftSize, true, true, false );
    ...
    // Multiply
    for( int i = 0; i < fft->complexBufferSize(); ++i ) fft->complexBuffer[i] *= std::conj( halfFFT[i] );
    ...
    for( int j = 0; j < n / 2; ++j ) 
        d[j] = powerTerms[0] + powerTerms[j] - 2 * fft->realBuffer[j] / n;

	return d;
	}

static std::vector<float> compute_dPrime( const float * in, Frame n, std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP )
	{
	// Get d
	std::vector<float> d = compute_d( in, n, fft, canceller );
...
float Audio::getTotalEnergy( flan_CANCEL_ARG_CPP ) const
	{
...
float Audio::getLocalWavelength( Channel channel, Frame start, Frame windowSize, std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP ) const
	{
	...
	const std::vector<float> dPrime = compute_dPrime( getSamplePointer( channel, start ), windowSize, fft, canceller );
...
std::vector<float> Audio::getLocalWavelengths( Channel channel, Frame start, Frame end, Frame windowSize, Frame hop, std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP ) const
	{
    ...
    for( Frame frame = start; frame + windowSize < end; frame += hop )
        out.push_back( getLocalWavelength( channel, frame, windowSize, fft, canceller ) );
        
    return out;
	}

float Audio::getAverageWavelength( Channel channel, float minActiveRatio, float maxLengthSigma, 
	Frame start, Frame end, Frame windowSize, Frame hop, std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP ) const
	{
	flan_PROCESS_START( 0 );
	return getAverageWavelength( getLocalWavelengths( channel, start, end, windowSize, hop, fft, canceller ), minActiveRatio, maxLengthSigma );
	}
...
float Audio::getAverageWavelength( const std::vector<float> & locals, float minActiveRatio, float maxLengthSigma ) const
	...
	const int numInvalids = std::count( locals.begin(), locals.end(), -1 );
	const int numValids = locals.size() - numInvalids;
	std::copy_if( locals.begin(), locals.end(), std::back_inserter( validLengths ), []( float x ){ return x != -1; } );

	vec2 msd = meanAndStandardDeviation( validLengths );
...
Frequency Audio::getLocalFrequency( Channel channel, Frame start, Frame windowSize, std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP ) const
	{
...
std::vector<Frequency> Audio::getLocalFrequencies( Channel channel, Frame start, Frame end, Frame windowSize, Frame hop, std::shared_ptr<FFTHelper> fft, flan_CANCEL_ARG_CPP ) const
    {
    ...
    std::vector<Frequency> out;
    for( Frame frame = start; frame + windowSize < end; frame += hop )
        out.push_back( getLocalFrequency( channel, frame, windowSize, fft, canceller ) );
