...
void Graph::drawWaveform( Func1x1 data, Rect rect, int plane, Color c, WaveformMode mode, flan_CANCEL_ARG_CPP )
	{
	flan_FUNCTION_LOG;
...
void Graph::drawWaveform( const float * data, int n, Rect rect, int plane, Color c, WaveformMode mode, flan_CANCEL_ARG_CPP )
	{
...
void Graph::drawWaveforms( std::vector<Func1x1> fs, Rect rect, int startPlane, WaveformMode mode, flan_CANCEL_ARG_CPP )
	{
...
void Graph::drawWaveforms( std::vector<const float *> fs, int n, Rect rect, int startPlane, WaveformMode mode, flan_CANCEL_ARG_CPP )
	{
	for( int f = 0; f < fs.size(); ++f )
		{
		const Color c = Color::fromHSV( 360.0f * f / fs.size(), .8, .65 );
...
void Graph::drawSpectrogram( Func2x1 data, Rect rect, int plane, float hue, flan_CANCEL_ARG_CPP )
	{
	flan_FUNCTION_LOG;
...
void Graph::drawSpectrogram( const float * data, int n, int m, Rect rect, int plane, float hue, flan_CANCEL_ARG_CPP )
	{
...
void Graph::drawSpectrograms( std::vector<Func2x1> fs, Rect rect, int startPlane, flan_CANCEL_ARG_CPP )
	{
	for( int f = 0; f < fs.size(); ++f )
		{
...
void Graph::drawSpectrograms( std::vector<const float *> fs, int n, int m, Rect rect, int startPlane, flan_CANCEL_ARG_CPP )
	{
	for( int f = 0; f < fs.size(); ++f )
		{
		const float hue = 360.0f * f / fs.size();
...
void Graph::drawFunction( const Func1x1 & f, Interval domain, int plane, Color c, flan_CANCEL_ARG_CPP )
	{
	flan_FUNCTION_LOG;
...
void Graph::drawFunctions( const std::vector<Func1x1> & fs, std::vector<Interval> domains, int plane, flan_CANCEL_ARG_CPP )
	{
...
void Graph::drawFloat( vec2 pos, Pixel digitWidth, Pixel digitHeight, float number, int plane, Color c, flan_CANCEL_ARG_CPP )
	{
	// Digit seperation
