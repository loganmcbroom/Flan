...
AudioBuffer AudioBuffer::deepCopy() const
	{
	AudioBuffer out;
	out.format = format;
...
bool AudioBuffer::isNull() const
	{
	return !buffer || buffer->empty() || getSampleRate() == 0;
	}
...
auto AudioBuffer::getSample_s( Channel channel, Frame frame ) const -> Sample
	{
	const size_t bufferPos = getBufferPos( channel, frame );
	if( 0 <= bufferPos && bufferPos < buffer->size() )
...
std::vector<Sample>::iterator AudioBuffer::begin()
	{
	return buffer->begin();
	}

std::vector<Sample>::iterator AudioBuffer::end()
	{
	return buffer->end();
	}

std::vector<Sample>::const_iterator AudioBuffer::begin() const
	{
	return buffer->begin();
	}

std::vector<Sample>::const_iterator AudioBuffer::end() const
	{
	return buffer->end();
	}
...
uint32_t AudioBuffer::getBufferPos( Channel channel, Frame sample ) const
	{
